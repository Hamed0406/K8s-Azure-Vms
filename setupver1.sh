...
sshpass -p $PASS ssh $USERNAME@$IP
...
loc=eastus
rg='rg-cka'
# storageacc=wgckastorage20230214
vmimage=ubuntults
shutdownutc=1630 #Auto Shutdown time in UTC example 16:30 .  
...
controllervmsize='Standard_B4ms' # check the available vm sizes in your region
size='Standard_B4ms'
...
#az network --public-ip-sku Standard create -n $clientpip -g $rg

az network public-ip create  --resource-group $rg --name $clientpip --version IPv4 --sku Standard --zone 1 2 3
...
installk8s $adminuser $adminpwd $clientpip
...
echo "-----------------  Creating Resourses DONE -----------------"


echo "----------------- Install K8s on All nodes  -----------------" 
